...
async def tour_type_selected(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not count_people or not season:
            await message.answer("❌ Не удалось получить данные поиска. Пожалуйста, начните заново.")
            await state.clear()
            return
        ...
        if tours:
            ...
            await message.answer("😔 К сожалению, по вашим критериям туров не найдено")
    
    except Exception as e:
        await message.answer(f"⚠️ Произошла ошибка: {str(e)}")
        print(f"Ошибка при поиске туров: {e}")
